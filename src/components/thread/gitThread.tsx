...
export function GitThread(props: { thread: ThreadElement[] }) {
  ...
  useEffect(() => {
    ...
    const threadBoundingClients = props.thread.map((x) => ({
      ...screenToCanvasSpace(
        canvas,
        x.relativeTo.current!.getBoundingClientRect(),
      ),
      origin: x,
    }));
    ...
    for (let i = 0; i < threadBoundingClients.length; i++) {
      const threadBoundingClient = threadBoundingClients[i];
      ...
      if (inLine) {
        const gradient = ctx.createLinearGradient(
          pointX,
          getY(threadBoundingClients[i - 1]),
          pointX,
          point.y,
        );
        addStops(
          gradient,
          threadBoundingClients[i - 1].origin,
          threadBoundingClient.origin,
        );
        ctx.lineWidth = 4;
    ...
    for (const threadBoundingClient of threadBoundingClients) {
      const point = {
        x: pointX,
        y: getY(threadBoundingClient),
      };
